...
const Gallery = ({ route }) => {
    ...
    const LeftContent = props => <Avatar.Image {...props} source={userData.profilePic ? { uri: `http://${IP}/EM_API/UserImages/${userData.profilePic}` } : require('../../assets/user.png')} />
    const DeleteIcon = ({ id }) => (
        <TouchableOpacity style={{ flexDirection: "row", alignItems: "center", backgroundColor: COLORS.light, padding: 5, borderRadius: 10 }}>
            <Pressable onPress={() => { handleDeletePic(id) }}>
    ...
    const handleDeletePic = async (id) => {
        try {
            ...
            const response = await fetch(
                `http://${IP}/EM_API/api/ProgressPhotos/deleteProgressPhotos?id=${id}`,
                {
                    method: 'Delete',
                },
            )
