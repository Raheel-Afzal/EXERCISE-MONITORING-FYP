...
import numpy as np
import datetime
from dbHandler import dbContext
...
def findAngle(img, lm1, lm2, lm3, bodyAngle, direction, exercise_name, draw=True):
    ...
    # Draw
    if draw and bodyAngle == 'dontDetect':
        cv2.putText(img, str(int(angle)), (x2, y2-30),
    ...
    if draw and bodyAngle == 'hipAngle':
        ...
        cv2.putText(img, f'v{vertex+3}', (x2, y2 - 10),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)
        cv2.putText(img, f'v{vertex+4}', (x3, y3 - 10),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)

        if exercise_name == 'pushUps':
            cv2.putText(img, f"{str(int(angle))}|175-180", (x2 - 50, y2 - 50),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2)
        if exercise_name == 'pushDowns':
            cv2.putText(img, f"{str(int(angle))}|175-180", (x2 + 20, y2 + 20),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2)
        elif exercise_name == 'benchDips':
            cv2.putText(img, f"{str(int(angle))}|100-130", (x2 - 70, y2+10),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2)
        elif exercise_name == 'tricepsKickBack':
            cv2.putText(img, f"{str(int(angle))}|140-130", (x2 - 80, y2+30),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), 2)
    ...
    if draw and bodyAngle == 'armAngle':
        ...
        cv2.putText(img, f'v{vertex+2}', (x1, y1 - 10),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)
        cv2.putText(img, f'v{vertex+1}', (x2, y2 - 10),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)
        cv2.putText(img, f'v{vertex}', (x3, y3 - 10),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (255, 255, 255), 2)

        if exercise_name == 'pushUps':
            cv2.putText(img, f"{str(int(angle))}| {sign} {armText}", (x2 - 50, y2 + 50),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 225), 2)
        if exercise_name == 'benchDips':
            cv2.putText(img, f"{str(int(angle))}| {sign} {armText}", (x2 - 70, y2+10),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 225), 2)
        if exercise_name == 'pushDowns':
            cv2.putText(img, f"{str(int(angle))}| {sign} {armText}", (x2 + 20, y2),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 225), 2)
        if exercise_name == 'tricepsKickBack':
            cv2.putText(img, f"{str(int(angle))}| {sign} {armText}", (x2 - 70, y2-20),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 225), 2)
        ...
        cv2.putText(img, f"{str(int(angle))}| {sign} {legText}", (x2+20, y2-20),
                    cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 225), 2)
...
def upload():
    ...
    while True:
        ...
        image_filename = f'image_{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}{i}.jpg'
  ...
  #  1  # ---------------->>>>> PUSHUPS <<<<<------------------- #  1  #
        ...
        if len(lmList) != 0:
            if exercise_name == 'pushUps':
                ...
                elbow_shoulder_hip, correct_form_arm = findAngle(
                    img, elbow, shoulder, hip, 'dontDetect', direction, exercise_name)
                    ...
                    if per == 0:
                        if shoulder_elbow_wrist <= 90 and shoulder_hip_ankle > 170:
                            ...
                            if correct_form_hip:
                                ...
                                db.addWrongPoseLog(userId, {
                                    'atRep': count,
                                    'wrongPosePic': image_filename,
                                    'direction': 'Down',
                                    'exerciseName': exercise_name,
                                    'angleOne'  : 'shoulder_elbow_wrist',
                                    'angleTwo'  : 'shoulder_hip_ankle',
                                    'wrongAngleOne': shoulder_elbow_wrist,
                                    'wrongAngleTwo': shoulder_hip_ankle,                                   
                                    'correctAngleOne': ' > 160',
                                    'correctAngleTwo': ' > 170',
                                    'bodyAngle': 'Arm Angle',
                                    'vertix1': vertexName1,
...
#  2  # ---------------->>>>> PUSH DOWN <<<<<------------------- #  2  #
 ...
 #  5  # ---------------->>>>> BenchDips <<<<<------------------- #  5  #
  ...
  # ---------------->>>>> Common Logic <<<<<-------------------
            ...
            rectangle_y = height - 250  # Distance from the bottom of the image
            rectangle_top_left = (0, rectangle_y)
            rectangle_bottom_right = (width, height-250)

            # Update the coordinates based on the rectangle position and size
            # Adjust the x-coordinate as needed
            text_x = rectangle_top_left[0] + 8
            # Adjust the y-coordinate as needed
            text_y = rectangle_top_left[1] - 50

            # Place the first text inside the rectangle
            cv2.putText(img, 'v1: '+vertexName1, (text_x, text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(img, 'v2: '+vertexName2, (text_x, text_y+30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(img, 'v3: '+vertexName3, (text_x, text_y+60),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(img, 'v4: '+vertexName4, (text_x, text_y+90),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(img, 'v5: '+vertexName5, (text_x, text_y+120),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            ...
            cv2.rectangle(img, rectangle_top_left,
                          rectangle_bottom_right, (225, 105, 94), cv2.FILLED)

            # Update the coordinates based on the rectangle position and size
            # Adjust the x-coordinate as needed
            text_x = rectangle_top_left[0] + 8
            # Adjust the y-coordinate as needed
            text_y = rectangle_top_left[1] + 35

            # Place the first text inside the rectangle
            cv2.putText(img, 'Reps: ' + str(int(count)), (text_x, text_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

            # Update the coordinates for the second text
            # Adjust the x-coordinate as needed
            feedback_x = rectangle_top_left[0] + 250
            # Adjust the y-coordinate as needed
            feedback_y = rectangle_top_left[1] + 30

            # Place the second text inside the rectangle
            cv2.putText(img, 'Feed Back: ' + feedback, (feedback_x, feedback_y),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    start_video.release()
    cap.release()
    db.addExerciseLog(userId, {'exerciseName': exercise_name, 'reps': count})
    # Send a response indicating success
    return {'message': 'Video uploaded successfully', 'userId': userId, 'exercise_name': exercise_name, 'reps': count}
...
def getImage(name):
    return send_file(name, 'image/jpg')


if __name__ == '__main__':
    app.run(host='192.168.43.251', port=8000)
